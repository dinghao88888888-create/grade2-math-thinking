...
- 彩色图示题：钟面、数轴、分组盒、彩灯周期、数位车、计数器、购物小票、座位图、故事分镜
- 题型包含看图列式、选择判断、纠错、估算、路线比较、多条件筛选和开放解释
- 支持记录“做对了 / 还要练”
- 本地错题本，可再次生成相似题
- 家长概览，查看薄弱单元和练习记录
- 打印当前练习题
...
Cloudflare Pages 已连接 GitHub 后，每次更新流程是：

1. 修改本地文件
2. 把改动提交到 GitHub 的 `main` 分支
3. Cloudflare Pages 自动检测到新提交并重新部署
4. 等部署完成后，线上网站自动变成新版

如果临时不用 Git 工具，也可以在 GitHub 网页里直接上传并替换同名文件，再提交更改。
